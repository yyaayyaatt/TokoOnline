    ...
    <section class="content">
      ...
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">Penilaian Produk</h3>
        </div>
        <!-- /.card-header -->
        <div class="card-body p-2">
          <ul class="products-list product-list-in-card pl-2 pr-2">
            <?php
            $rates = mysqli_query($conn, "SELECT rate,pesan,pelanggan.nama FROM rating  
                        INNER JOIN pelanggan on pelanggan.id_pel=rating.pelanggan 
                        WHERE rating.produk = " . $row['id_produk']);
            $rate = 0;
            while ($data = mysqli_fetch_array($rates)) {
              $rate = $data['rate'];
              settype($rate, "integer");
            ...
            ?>
              <li class="item">
                <div class="product-info">
                  <a href="javascript:void(0)" class="product-title"><?php echo $data['nama'] ?>
